...
from .config import default_download_dir as config_default_download_dir
from .config import storage_root as config_storage_root
...
def storage_root() -> Path:
    return config_storage_root()


def default_download_dir() -> Path:
    return config_default_download_dir()
...
def detect_kind(text: str, explicit_kind: str | None = None) -> str:
    ...
    if any(term in lowered for term in BOOK_TERMS):
        return "book"
    if extract_year(lowered) and (extract_english_alias(text) or has_chinese(text)):
        return "movie"
    return "general"
