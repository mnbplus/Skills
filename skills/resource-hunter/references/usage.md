...
- `packaging-baseline-report` consumes one archived `packaging-baseline.json`, multiple explicit baseline files, or directories that are scanned recursively for `packaging-baseline.json` so downloaded CI artifact trees can be consumed without ad-hoc shell loops. Single-artifact input keeps the normalized `artifact_path` + `captures` view; multi-artifact input emits an aggregate report with top-level contract counts, prefixed warnings, and nested per-artifact reports. Add `--json` for machine-readable output, and add `--require-contract-ok` when downstream automation should exit with code `2` after printing the report if any archived artifact drifts from the expected passing-vs-blocked contract.
- Direct Python consumers can now import `resource_hunter.packaging_report.read_packaging_baseline_report()` or `resource_hunter.packaging_report.build_packaging_baseline_report()` to get the same normalized single-artifact envelope without shelling out or re-parsing `passing_capture` / `blocked_capture`.
