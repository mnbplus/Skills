...
- `packaging-baseline-report` consumes one archived `packaging-baseline.json`, multiple explicit baseline files, `.zip` archives that contain one or more `packaging-baseline.json` members, or directories that are scanned recursively for `packaging-baseline.json` so downloaded CI artifact trees can be consumed without ad-hoc shell loops. Single-artifact input keeps the normalized `artifact_path` + `captures` view; multi-artifact input emits an aggregate report with top-level contract counts, prefixed warnings, and nested per-artifact reports. Zip-backed artifacts are rendered as `archive.zip!/inner/path/packaging-baseline.json` so logs still point at the original archive member. Add `--json` for machine-readable output, and add `--require-contract-ok` when downstream automation should exit with code `2` after printing the report if any archived artifact drifts from the expected passing-vs-blocked contract.
- Direct Python consumers can now import `resource_hunter.packaging_report.read_packaging_baseline_report()`, `resource_hunter.packaging_report.read_packaging_baseline_reports()`, or `resource_hunter.packaging_report.build_packaging_baseline_report()` to get the same normalized single-artifact envelope or aggregate report without shelling out or re-parsing `passing_capture` / `blocked_capture`. When a downstream job only needs the read-only gate result, `resource_hunter.packaging_gate.evaluate_packaging_baseline_gate()` wraps that report API together with `packaging_baseline_report_requirement_failures()` and now returns a versioned payload via `gate_schema_version`, while the installed `resource-hunter-packaging-baseline-gate` entrypoint, `python -m resource_hunter.packaging_gate`, and `python3 scripts/packaging_gate.py` surface the same summary for release jobs and source checkouts.
